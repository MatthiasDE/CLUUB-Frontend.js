<template>
  <div class="px-4 sm:px-0">
    <div class="bg-white shadow rounded-lg p-6">
      <h2 class="text-2xl font-bold text-gray-900 mb-6">Time Recording</h2>
      ...
      <!-- Success Message -->
      <div v-if="successMessage" class="mb-4 bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded relative" role="alert">
      ...
      <form @submit.prevent="submitTimeRecord" class="space-y-6">
        ...
        <div>
          <label for="member-search" class="block text-sm font-medium text-gray-700 mb-1">
            Member
          </label>
          <div class="relative">
            <input
            ...
            <div v-if="selectedMember" class="mt-2 text-sm text-gray-600">
              Selected: <span class="font-medium">{{ selectedMember.first_name }} {{ selectedMember.name }}</span> (ID: {{ selectedMember.member_id }})
            </div>
          </div>
        </div>
        ...
        <!-- Start Date and Time -->
        <div>
          <label for="worktime-from" class="block text-sm font-medium text-gray-700 mb-1">
            Start Date and Time
          </label>
          <input
            id="worktime-from"
        ...
        <!-- End Date and Time -->
        <div>
          <label for="worktime-to" class="block text-sm font-medium text-gray-700 mb-1">
            End Date and Time
          </label>
          <input
            id="worktime-to"
    ...
    <div class="mt-8 bg-white shadow rounded-lg p-6">
      <h3 class="text-xl font-bold text-gray-900 mb-4">Today's Time Records</h3>

      <div v-if="isLoadingRecords" class="text-center py-4 text-gray-500">
        Loading records...
      </div>

      <div v-else-if="todayRecords.length === 0" class="text-center py-4 text-gray-500">
      ...
      <div v-else class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="record in todayRecords" :key="record.record_id">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {{ calculateDuration(record.worktime_from, record.worktime_to) }}
              </td>
...
export default {
  ...
  methods: {
    ...
    async searchMembers() {
      try {
        const response = await fetch(`/api/v1/members?search=${encodeURIComponent(this.searchQuery)}&fuzzy=true`)
        if (!response.ok) {
        ...
        this.errorMessage = 'Failed to search members. Please try again.'
    ...
    async submitTimeRecord() {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(errorData.detail || 'Failed to create time record')
        }
        ...
        this.errorMessage = error.message || 'Failed to create time record. Please try again.'
      } finally {
    ...
    async loadTodayRecords() {
      ...
      try {
        ...
        const response = await fetch(`/api/v1/timerecords?start_date=${encodeURIComponent(todayISO)}`)
        if (!response.ok) {
    ...
    calculateDuration(from, to) {
      ...
      if (!fromString.endsWith('Z') && !fromString.includes('+') && !fromString.includes('-', 10)) {
        fromString += 'Z'
      }
      if (!toString.endsWith('Z') && !toString.includes('+') && !toString.includes('-', 10)) {
        toString += 'Z'
      }
