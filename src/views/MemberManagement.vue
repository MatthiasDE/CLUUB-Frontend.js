<template>
  <div class="px-4 sm:px-0">
    <div class="bg-white shadow rounded-lg p-6 max-w-2xl">
      <h2 class="text-2xl font-bold text-gray-900 mb-6">Member Management</h2>
      ...
      <form @submit.prevent="submitMember" class="space-y-6">
        ...
        <div>
          ...
          <input
            ...
            required
          />
          <p class="mt-1 text-sm text-gray-500">Unique identifier for the member (must be positive integer)</p>
        </div>
  ...
  methods: {
    async submitMember() {
      ...
      if (!this.memberId || this.memberId < 1) {
        this.errorMessage = 'Please enter a valid member ID (positive integer)'
        return
      }
